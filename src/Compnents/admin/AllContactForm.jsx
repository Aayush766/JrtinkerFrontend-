...
    setLoading(true);
    try {
      // FIX: Corrected the URL and added { withCredentials: true }
      const res = await axios.get("https://jrtinker01.onrender.com/admin/dashboard/contact-forms", { withCredentials: true });

      let fetchedSubmissions = [];
      if (res.data && Array.isArray(res.data.contactForms)) {
        fetchedSubmissions = res.data.contactForms;
      } else if (Array.isArray(res.data)) {
        fetchedSubmissions = res.data;
      } else {
        console.warn(
          "API response for contact submissions is not an array, received:",
          res.data
        );
        fetchedSubmissions = [];
      }
      setContactSubmissions(fetchedSubmissions);
    } catch (err) {
      console.error("Error fetching contact submissions:", err);
      setError(err.response?.data?.message || "Failed to fetch contact submissions.");
      toast.error(err.response?.data?.message || "Failed to fetch contact submissions.");
...
      try {
        // Note: You'll need to create a DELETE endpoint on your backend for this to work
        await axios.delete(`https://jrtinker01.onrender.com/admin/submissions/contact/${id}`, { withCredentials: true });
        toast.success(`Submission from "${name}" deleted successfully!`);
...
          ></path>
        </svg>
        <p className="ml-4 text-xl text-gray-700">Loading contact submissions...</p>
       </div>
    );
  }
...
            No general contact forms have been received yet.
          </p>
        </div>
      ) : (
